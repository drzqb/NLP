'''
    word2vec skip-gram model based on tensorflow
'''
...
class Lang():
    ...
    def make_data(self, len_text, text2ind):
        self.y_train = np.array([[text2ind[i + k] for k in range(FLAGS.window_size)] +
                                 [text2ind[i + FLAGS.window_size + 1 + k] for k in range(FLAGS.window_size)]
                                 for i in range(0, len_text - 2 * FLAGS.window_size)])
        self.x_train = np.array([text2ind[i] for i in range(FLAGS.window_size, len_text - FLAGS.window_size)])
...
class CBOW():
    ...
    def build(self):
        with tf.name_scope('Input'):
            self.train_inputs = tf.placeholder(tf.int32, [None], name='word')
            self.train_labels = tf.placeholder(tf.int32, [None, 2 * FLAGS.window_size], name='context')
        ...
        with tf.name_scope('Output'):
            w1 = tf.Variable(
                tf.truncated_normal([FLAGS.embed_size, FLAGS.vocab_size], stddev=1.0 / np.sqrt(FLAGS.vocab_size)),
                name='weights')
            b1 = tf.Variable(tf.zeros([FLAGS.vocab_size]), name='biases')
            output = tf.nn.log_softmax(tf.matmul(embed, w1) + b1)

        with tf.name_scope('Loss'):
            with tf.name_scope('Indice'):
                indice = tf.reshape(tf.range(0, tf.shape(self.train_inputs)[0], dtype=tf.int32),
                                    [tf.shape(self.train_inputs)[0], 1])
                indice = tf.concat([indice for _ in range(2 * FLAGS.window_size)], axis=1)
                index1 = tf.reshape(self.train_labels, [tf.shape(self.train_inputs)[0], 2 * FLAGS.window_size, 1])
                index2 = tf.reshape(indice, [tf.shape(self.train_inputs)[0], 2 * FLAGS.window_size, 1])
                index3 = tf.concat([index2, index1], axis=2)

            output1 = -tf.gather_nd(output, index3)
            self.loss = tf.reduce_mean(tf.reduce_sum(output1, axis=1), name='loss')
    ...
    def train(self, x_train, y_train, ind2word):
        ...
        for epoch in range(1, FLAGS.epochs + 1):
            ...
            plt.savefig(FLAGS.skipgram_save, bbox_inches='tight')
            plt.close(fig)

        fig = plt.figure(figsize=(10, 10))
        plt.plot(loss_)
        plt.savefig(FLAGS.loss_skipgram_save, bbox_inches='tight')
        plt.close(fig)
