...
class NLI():
    ...
    def build_model(self, embed_matrix):
        with tf.name_scope('input'):
            ...
            padding_mask_p = tf.tile(tf.expand_dims(tf.greater(p, 0), 1), [1, tf.shape(p)[1], 1], name='mask_p')
            padding_mask_h = tf.tile(tf.expand_dims(tf.greater(h, 0), 1), [1, tf.shape(h)[1], 1], name='mask_h')
        ...
        for block in range(self.config.block):
            with tf.name_scope('selfattention' + str(block)):
                ...
                QKF = [tf.ones_like(QK[i]) ** (1. - tf.pow(2., 31.)) for i in range(self.config.head)]
                ...
                Z_h = tf.nn.dropout(WO(tf.concat(
                    [tf.matmul(tf.nn.softmax(tf.where(padding_mask_h, QK[i], QKF[i]), axis=-1), V[i]) for i in
                     range(self.config.head)], axis=-1)), keep_prob)
