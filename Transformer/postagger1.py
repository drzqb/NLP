...
class PosTagger():
    ...
    def build_model(self):
        ...
        with tf.name_scope('loss'):
            ...
            accuracy = tf.cast(tf.equal(prediction, tag), tf.float32)
            ...
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=tag, logits=logits)
            lossf = tf.zeros_like(loss)
            loss = tf.reduce_mean(
                tf.div(tf.reduce_sum(tf.where(sequence_mask, loss, lossf), axis=-1), tf.cast(length, tf.float32)),
                name='loss')

            optimizer = tf.train.AdamOptimizer(learning_rate=self.config.lr, name='optimizer')
            train_op = optimizer.minimize(loss, name='train_op')
        writer = tf.summary.FileWriter('logs', graph=tf.get_default_graph(), filename_suffix='mypt2')
        writer.flush()
        writer.close()
        ...
        saver.save(sess, self.config.model_save_path + 'mypt2')
        sess.close()
    ...
    def train(self):
        ...
        newsaver = tf.train.import_meta_graph(self.config.model_save_path + 'mypt2.meta')
        newsaver.restore(sess, self.config.model_save_path + 'mypt2')
        ...
        for epoch in range(1, self.config.epochs + 1):
            ...
            if epoch % self.config.per_save == 0:
                ...
                newsaver.save(sess, self.config.model_save_path + 'mypt2')
                print('model saved successfully!')
        ...
        plt.plot(loss_)
        plt.savefig(self.config.model_save_path + 'mypt2_loss.png')
        plt.close(fig)
