'''
    Transformer model for chatbot with simplified weights
'''
import numpy as np
...
class Transformer():
    ...
    def build_model(self):
        ...
        with tf.name_scope('selfattention'):
            WQ_sa = [tf.layers.Dense(self.config.embedding_qa_size, use_bias=False, name='Q_sa') for _ in
                  range(self.config.block)]
            WK_sa = [tf.layers.Dense(self.config.embedding_qa_size, use_bias=False, name='K_sa') for _ in
                  range(self.config.block)]
            WV_sa = [tf.layers.Dense(self.config.embedding_qa_size, use_bias=False, name='V_sa') for _ in
                  range(self.config.block)]
            WO_sa = [tf.layers.Dense(self.config.embedding_qa_size, use_bias=False, name='O_sa') for _ in
                  range(self.config.block)]

        with tf.name_scope('encoder_decoder_attention'):
            WQ_eda = [tf.layers.Dense(self.config.embedding_qa_size, use_bias=False, name='Q_eda') for _ in
                  range(self.config.block)]
            WK_eda = [tf.layers.Dense(self.config.embedding_qa_size, use_bias=False, name='K_eda') for _ in
                  range(self.config.block)]
            WV_eda = [tf.layers.Dense(self.config.embedding_qa_size, use_bias=False, name='V_eda') for _ in
                  range(self.config.block)]
            WO_eda = [tf.layers.Dense(self.config.embedding_qa_size, use_bias=False, name='O_eda') for _ in
                  range(self.config.block)]
        ...
        with tf.variable_scope('Encoder'):
            ...
            for block in range(self.config.block):
                with tf.variable_scope('selfattention' + str(block)):
                    Q = tf.concat(tf.split(WQ_sa[block](encoder_p), self.config.head, axis=-1), axis=0)
                    K = tf.concat(tf.split(WK_sa[block](encoder_p), self.config.head, axis=-1), axis=0)
                    V = tf.concat(tf.split(WV_sa[block](encoder_p), self.config.head, axis=-1), axis=0)

                    QK = tf.matmul(Q, tf.transpose(K, [0, 2, 1])) / tf.sqrt(
                        self.config.embedding_qa_size / self.config.head)
                    Z_p = WO_sa[block](tf.concat(
        ...
        with tf.variable_scope('Decoder'):
            ...
            with tf.variable_scope('decoder'):
                ...
                for block in range(self.config.block):
                    with tf.variable_scope('mask_selfattention' + str(block)):
                        Q = tf.concat(tf.split(WQ_sa[block](decoder_p), self.config.head, axis=-1), axis=0)
                        K = tf.concat(tf.split(WK_sa[block](decoder_p), self.config.head, axis=-1), axis=0)
                        V = tf.concat(tf.split(WV_sa[block](decoder_p), self.config.head, axis=-1), axis=0)
                    ...
                    with tf.variable_scope('encoder_decoder_attention' + str(block)):
                        Q = tf.concat(tf.split(WQ_eda[block](decoder_p), self.config.head, axis=-1), axis=0)
                        K = tf.concat(tf.split(WK_eda[block](encoder_p), self.config.head, axis=-1), axis=0)
                        V = tf.concat(tf.split(WV_eda[block](encoder_p), self.config.head, axis=-1), axis=0)

                        QK = tf.matmul(Q, tf.transpose(K, [0, 2, 1])) / tf.sqrt(
                            self.config.embedding_qa_size / self.config.head)
                        Z_p = WO_eda[block](tf.concat(
                            tf.split(
            ...
            with tf.variable_scope('decoder', reuse=tf.AUTO_REUSE):
                ...
                def body(k, decoder_infer_inputs, decoder_infer_outputs):
                    ...
                    for block in range(self.config.block):
                        with tf.variable_scope('mask_selfattention' + str(block)):
                            Q = tf.concat(tf.split(WQ_sa[block](decoder_infer), self.config.head, axis=-1), axis=0)
                            K = tf.concat(tf.split(WK_sa[block](decoder_infer), self.config.head, axis=-1), axis=0)
                            V = tf.concat(tf.split(WV_sa[block](decoder_infer), self.config.head, axis=-1), axis=0)
                        ...
                        with tf.variable_scope('encoder_decoder_attention' + str(block)):
                            Q = tf.concat(tf.split(WQ_eda[block](decoder_infer), self.config.head, axis=-1), axis=0)
                            K = tf.concat(tf.split(WK_eda[block](encoder_p), self.config.head, axis=-1), axis=0)
                            V = tf.concat(tf.split(WV_eda[block](encoder_p), self.config.head, axis=-1), axis=0)

                            QK = tf.matmul(Q, tf.transpose(K, [0, 2, 1])) / tf.sqrt(
                                self.config.embedding_qa_size / self.config.head)
                            Z_p = WO_eda[block](tf.concat(tf.split(
        ...
        if FLAGS.graph_write:
            writer = tf.summary.FileWriter('logs', graph=tf.get_default_graph(), filename_suffix='chatbot4')
            writer.flush()
            writer.close()
        ...
        saver.save(sess, FLAGS.model_save_path + 'chatbot4')
        sess.close()
    ...
    def train(self, qtext2id, qtext2id_length, atext2id_input, atext2id_target, atext2id_input_length,
        ...
        new_saver = tf.train.import_meta_graph(self.config.model_save_path + 'chatbot4.meta')
        new_saver.restore(sess, self.config.model_save_path + 'chatbot4')
        ...
        for epoch in range(1, self.config.epochs + 1):
            ...
            if epoch % self.config.per_save == 0:
                new_saver.save(sess, self.config.model_save_path + 'chatbot4')
                print('Model saved successfully!')

        fig = plt.figure(figsize=(10, 8))
        plt.plot(loss_)
        plt.savefig(self.config.model_save_path + 'Transformer_Loss_chatbot4.png', bbox_inches='tight')
        plt.close(fig)

    def predict(self, qa_dict, qa_reverse_dict):
        sess = tf.Session()
        tf.train.import_meta_graph(self.config.model_save_path + 'chatbot4.meta').restore(sess,
                                                                                          self.config.model_save_path + 'chatbot4')
        graph = tf.get_default_graph()
