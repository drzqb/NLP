...
class Seq2Seq():
    ...
    def build_model(self):
        ...
        with tf.variable_scope('decoder'):
            ...
            W_attn = tf.get_variable('W_attention', [2 * FLAGS.decoder_hidden_units, FLAGS.encoder_hidden_units])

            output_layer = tf.layers.Dense(self.l_dict_ch, name='projection_layer')

            with tf.variable_scope('decoder'):
                training_decoder_outputs = tf.zeros([0, FLAGS.decoder_hidden_units])
                ...
                def body_time(i, training_decoder_outputs, decoder_initial_state):
                    e_i = tf.matmul(decoder_initial_state, W_attn)
                    e_ij = tf.matmul(tf.reshape(e_i, [-1, 1, 1, FLAGS.encoder_hidden_units]),
                                     tf.expand_dims(tf.transpose(encoder_outputs, (0, 2, 1)), 1))

                    j = tf.constant(0, tf.int32)
                    weight = tf.zeros([0], tf.float32)

                    def cond_batch(j, weight):
                        return tf.less(j, tf.shape(encoder_inputs)[0])

                    def body_batch(j, weight):
                        weight = tf.concat([weight, tf.nn.softmax(e_ij[j, 0, 0, :encoder_inputs_length[j]])], axis=0)
                        weight = tf.concat([weight, tf.zeros([max_encoder_inputs_length - encoder_inputs_length[j]])],
                                           axis=0)
                        return tf.add(j, 1), weight

                    _, weight = tf.while_loop(cond_batch, body_batch, [j, weight],
                                              shape_invariants=[j.get_shape(), tf.TensorShape([None])])

                    weight = tf.reshape(weight, [-1, 1, 1, max_encoder_inputs_length])

                    context = tf.reshape(tf.matmul(weight, tf.expand_dims(encoder_outputs, 1)),
                                         [-1, FLAGS.encoder_hidden_units])
            ...
            with tf.variable_scope('decoder', reuse=tf.AUTO_REUSE):
                ...
                def body_time_pre(i, prediction, decoder_initial_state, decoder_input):
                    ...
                    def cond_batch_pre(j, weight):
                        return tf.less(j, tf.shape(encoder_inputs)[0])

                    def body_batch_pre(j, weight):
                        weight = tf.concat([weight, tf.nn.softmax(e_ij[j, 0, 0, :encoder_inputs_length[j]])], axis=0)
                        weight = tf.concat([weight, tf.zeros([max_encoder_inputs_length - encoder_inputs_length[j]])],
                                           axis=0)
                        return tf.add(j, 1), weight

                    _, weight = tf.while_loop(cond_batch_pre, body_batch_pre, [j, weight],
                                              shape_invariants=[j.get_shape(), tf.TensorShape([None])])
                    ...
                    prediction_decoder_output, decoder_initial_state = decoder_cell(
                        tf.concat([context, tf.nn.embedding_lookup(embedding_matrix_ch, decoder_input)], axis=-1),
        ...
        if FLAGS.graph_write:
            writer = tf.summary.FileWriter('logs', graph=tf.get_default_graph(), filename_suffix='nmt1')
            writer.flush()
            writer.close()
        ...
        saver.save(sess, FLAGS.model_save_path + 'nmt1')
        sess.close()
    ...
    def train(self, entext2id, entext2id_length, en_dict, chtext2id_input, chtext2id_input_length, ch_dict,
        ...
        new_saver = tf.train.import_meta_graph(FLAGS.model_save_path + 'nmt1.meta')
        new_saver.restore(sess, FLAGS.model_save_path + 'nmt1')
        ...
        for epoch in range(1, FLAGS.epochs + 1):
            ...
            if epoch % FLAGS.per_save == 0:
                new_saver.save(sess, FLAGS.model_save_path + 'nmt1')
                print('Model saved successfully!')

        fig = plt.figure(figsize=(10, 8))
        plt.plot(loss_)
        plt.savefig(FLAGS.model_save_path + 'seq2seq_attention_loss.png', bbox_inches='tight')
        plt.close(fig)

    def predict(self, en_dict, ch_dict, ch_reverse_dict):
        sess = tf.Session()
        tf.train.import_meta_graph(FLAGS.model_save_path + 'nmt1.meta').restore(sess,
                                                                                   FLAGS.model_save_path + 'nmt1')
        graph = tf.get_default_graph()
