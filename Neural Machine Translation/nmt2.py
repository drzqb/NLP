...
class AttnDecoder(torch.nn.Module):
    def __init__(self, l_dict_ch):
        ...
        self.decoder_gru = GRU(input_size=FLAGS.embedding_ch_size + FLAGS.hidden_units,
                               hidden_size=FLAGS.hidden_units,
                               num_layers=FLAGS.n_layers,
                               dropout=FLAGS.drop_prob,
                               batch_first=True)

        self.attn = Linear(FLAGS.n_layers * FLAGS.hidden_units, FLAGS.hidden_units, bias=False)
        self.project = Linear(FLAGS.hidden_units, l_dict_ch)
...
class Usr():
    def train(self, entext2id, entext2id_length, en_dict,
        ...
        if FLAGS.mode == 'train1':
            encoder.load_state_dict(torch.load(FLAGS.model_save_path + 'encoder.pkl'))
            attndecoder.load_state_dict(torch.load(FLAGS.model_save_path + 'attndecoder.pkl'))
        ...
        for epoch in range(1, FLAGS.epochs + 1):
            ...
            if epoch % FLAGS.per_save == 0:
                torch.save(encoder.state_dict(), FLAGS.model_save_path + 'encoder.pkl')
                torch.save(attndecoder.state_dict(), FLAGS.model_save_path + 'attndecoder.pkl')

        fig = plt.figure(figsize=(10, 8))
        plt.plot(loss_)
        plt.savefig(FLAGS.model_save_path + 'seq2seq_attention_loss.png')
        plt.close(fig)
