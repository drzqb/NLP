...
tf.flags.DEFINE_string('model_save_path', 'model/fm1', 'directory of model file saved')
...
class FM():
    ...
    def build_model(self):
        ...
        with tf.name_scope('firstorder'):
            output1 = tf.reduce_sum(tf.squeeze(feature_embeded1, axis=-1), axis=-1)
...
def main(unused_argvs):
    ...
    with tf.Session() as sess:
        myfm = FM(CONFIG, feature_sizes, field_size)
        ...
        if CONFIG.mode.startswith('train'):
            if CONFIG.mode == 'train1':
                myfm.restore(sess, CONFIG.model_save_path)
            ...
            for epoch in range(1, CONFIG.epochs + 1):
                ...
                for batch in range(total_batch):
                    ...
                    loss_batch, acc_batch, auc_batch = myfm.train(sess, X_index, X_value, y)
                ...
                acc_val, auc_val = myfm.eval(sess, index_val, value_val, label_val)
                ...
                if epoch % CONFIG.per_save == 0:
                    myfm.save(sess, CONFIG.model_save_path)
        else:
            myfm.restore(sess, CONFIG.model_save_path)

            for batch in range(total_batch):
                X_index, X_value, y = get_batch(index, value, label, CONFIG.batch_size, batch)
                result = myfm.predict(sess, X_index, X_value)
                print(y)
                print(result)
                print('----------------------------------------------------------------')
