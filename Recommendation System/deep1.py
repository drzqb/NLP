...
tf.flags.DEFINE_string('model_save_path', 'model/deep1', 'directory of model file saved')
tf.flags.DEFINE_float('lr', 0.1, 'learning rate for training')
tf.flags.DEFINE_float('keep_prob', 0.5, 'rate for dropout')
tf.flags.DEFINE_float('l2', 0.01, 'rate for dropout')
...
class Deep():
    ...
    def build_model(self):
        ...
        with tf.name_scope('output'):
            out = tf.squeeze(tf.layers.dense(outputdeep, 1,
...
def main(unused_argvs):
    ...
    with tf.Session() as sess:
        mydeep = Deep(CONFIG, feature_sizes, field_size)
        ...
        if CONFIG.mode.startswith('train'):
            if CONFIG.mode == 'train1':
                mydeep.restore(sess, CONFIG.model_save_path)
            ...
            for epoch in range(1, CONFIG.epochs + 1):
                ...
                for batch in range(total_batch):
                    ...
                    loss_batch, acc_batch, auc_batch = mydeep.train(sess, X_index, X_value, y)
                ...
                acc_val, auc_val = mydeep.eval(sess, index_val, value_val, label_val)
                ...
                if epoch % CONFIG.per_save == 0:
                    mydeep.save(sess, CONFIG.model_save_path)
        else:
            mydeep.restore(sess, CONFIG.model_save_path)

            for batch in range(total_batch):
                X_index, X_value, y = get_batch(index, value, label, CONFIG.batch_size, batch)
                result = mydeep.predict(sess, X_index, X_value)
                print(y)
                print(result)
                print('----------------------------------------------------------------')
