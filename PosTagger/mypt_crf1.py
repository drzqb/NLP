...
class PosTagger():
    ...
    def build_model(self):
        ...
        writer = tf.summary.FileWriter('logs', graph=tf.get_default_graph(), filename_suffix='mypt_crf1')
        writer.flush()
        writer.close()
        ...
        saver.save(sess, self.config.model_save_path + 'mypt_crf1')
        sess.close()
    ...
    def train(self):
        ...
        newsaver = tf.train.import_meta_graph(self.config.model_save_path + 'mypt_crf1.meta')
        newsaver.restore(sess, self.config.model_save_path + 'mypt_crf1')
        ...
        for epoch in range(1, self.config.epochs + 1):
            ...
            if epoch % self.config.per_save == 0:
                ...
                newsaver.save(sess, self.config.model_save_path + 'mypt_crf1')
                print('model saved successfully!')
        ...
        plt.plot(loss_)
        plt.savefig(self.config.model_save_path + 'mypt_crf1_loss.png')
        plt.close(fig)
