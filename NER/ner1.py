...
tf.flags.DEFINE_string('model_save_path', 'model/ner1/', 'directory of model file saved')
...
tf.flags.DEFINE_string('mode', 'predict', 'The mode of train or predict as follows: '
...
class NER():
    ...
    def build_model(self):
        ...
        with tf.name_scope('loss'):
            ...
            accuracy = tf.cast(tf.equal(prediction, ner), tf.float32)
            accuracyf = tf.zeros_like(accuracy)
            accuracy = tf.where(masks, accuracy, accuracyf)
            accuracy = tf.divide(tf.reduce_sum(accuracy), tf.reduce_sum(tf.cast(length, tf.float32)), name='accuracy')

            loss = tf.identity(tf.contrib.seq2seq.sequence_loss(logits, ner, tf.cast(masks, tf.float32)), name='loss')
