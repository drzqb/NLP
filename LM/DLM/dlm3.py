...
tf.flags.DEFINE_string('model_save_path', 'model/dlm3/', 'The path where model shall be saved')
...
tf.flags.DEFINE_integer('pri', 200, 'topk choices')
...
class DLM():
    ...
    def build_model(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            for block in range(self.config.block):
                with tf.variable_scope('selfattention' + str(block)):
                    ...
                    scale_sa = tf.get_variable('scale_sa/layer_norm',
                                               initializer=tf.ones([self.config.embedding_size], dtype=tf.float32))
                    bias_sa = tf.get_variable('bias_sa/layer_norm',
                    ...
                    Z = WO(tf.concat(
                        tf.split(tf.matmul(tf.nn.dropout(softmax(QK, mask_final), keep_prob), V), self.config.head,
                                 axis=0), axis=-1))
                    now1 = tf.nn.dropout(layer_norm_compute(now1 + Z, scale_sa, bias_sa), keep_prob)
                ...
                with tf.variable_scope('feedforward' + str(block)):
                    ...
                    bias_ff_sen = tf.get_variable('bias_ff_sen/layer_norm',
                                                  ...
                                                  initializer=tf.zeros_initializer(),
                                                  dtype=tf.float32)
                    now1 = tf.nn.dropout(layer_norm_compute(ff(ffrelu(now1)) + now1, scale_ff_sen, bias_ff_sen),
                                         keep_prob)
        ...
        with tf.variable_scope('decoder'):
            ...
            for block in range(self.config.block):
                ...
                with tf.variable_scope('feedforward' + str(block)):
                    ...
                    now2 = tf.nn.dropout(layer_norm_compute(ff(ffrelu(now2)) + now2, scale_ff_pos, bias_ff_pos),
                                         keep_prob)
        ...
        bias_pr = tf.get_variable('bias_pr/layer_norm',
                                  ...
                                  initializer=tf.zeros_initializer(),
                                  dtype=tf.float32)
        now2 = layer_norm_compute(tf.layers.dense(now2,
                                                  self.config.embedding_size,
                                                  kernel_initializer=tf.truncated_normal_initializer(stddev=0.02),
                                                  activation=gelu),
                                  scale_pr, bias_pr)
