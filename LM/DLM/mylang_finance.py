...
class Lang():
    ...
    def read_data(self):
        fc = open('data/finance.txt', 'w', encoding='utf-8')
        m_samples = 0
        ...
        for i in range(nrows - 1):
            ...
            for sentence in sentences:
                ...
                #     sentence += '。'
                if len(sentence) <= self.config.maxword and len(sentence) > 1:
                    fc.write(sentence + '\n')
                    m_samples += 1
    ...
    def toid(self):
        ...
        writer = tf.python_io.TFRecordWriter('data/train_finance.tfrecord')
        max_len = 0
        with  open('data/finance.txt', 'r', encoding='utf-8') as f:
            for line in tqdm(f):
                line = line.strip()
