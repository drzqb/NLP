'''
    ...
    sen and pos 使用encoder-decoder模式
    encoder使用bert
'''
...
tf.flags.DEFINE_integer('block', 12, 'number of Encoder submodel')
tf.flags.DEFINE_integer('head', 12, 'number of multi_head attention')
tf.flags.DEFINE_string('model_save_path', 'model/dlm4/', 'The path where model shall be saved')
tf.flags.DEFINE_string('checkpoint', 'dlm.ckpt', 'The path where model shall be saved')
...
tf.flags.DEFINE_float('lr', 1.0e-4, 'learning rate for training')
...
def layer_norm_compute(x, gamma, beta, epsilon=1.0e-6):
...
def softmax(A, Mask):
    ...
    :return: C
    '''
    A = tf.where(Mask, A, tf.ones_like(A) * (1. - tf.pow(2., 31.)))
    A = tf.math.exp(A)
    B = tf.reduce_sum(A, axis=-1, keepdims=True)
    return tf.div_no_nan(A, B)
...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps):
    ...
    optimizer = AdamWeightDecayOptimizer(
        ...
        beta_2=0.999,
        epsilon=1e-6,
        exclude_from_weight_decay=["LayerNorm", "bias", "output_bias"])

    # tvars1 = tf.trainable_variables(scope='decoder')
    # tvars2 = tf.trainable_variables(scope='cls')
    # tvars = tvars1 + tvars2
...
class DLM():
    ...
    def build_model(self):
        ...
        ls = tf.placeholder(tf.int32, [None], name='ls')  #去掉CLS后的长度
        ...
        pos = tf.tile(tf.expand_dims(pos_full[:max_ls], axis=0), [2 * batch_size, 1, 1])

        with tf.variable_scope('bert'):
            sensen = tf.concat([sen, sen], axis=0)
            padding_mask = tf.tile(tf.expand_dims(tf.greater(sensen, 0), 1),
                                   [self.config.head, max_ls, 1])
            future_mask = tf.tile(
                tf.expand_dims(tf.sequence_mask(tf.range(1, limit=max_ls + 1), max_ls), 0),
                [batch_size, 1, 1])
            ...
            with tf.variable_scope('embeddings'):
                embedding_matrx = tf.get_variable('word_embeddings',
                                                  shape=[self.char_dict_len, self.config.embedding_size],
                                                  initializer=tf.truncated_normal_initializer(stddev=0.02),
                                                  dtype=tf.float32)
                embedded_sen = tf.nn.embedding_lookup(embedding_matrx, sensen)
                scale_emb_sen = tf.get_variable('LayerNorm/gamma',
                                                [self.config.embedding_size],
                                                initializer=tf.ones_initializer(),
                                                dtype=tf.float32)
                bias_emb_sen = tf.get_variable('LayerNorm/beta',
                                               [self.config.embedding_size],
                                               initializer=tf.zeros_initializer(),
                                               dtype=tf.float32)
                now1 = layer_norm_compute(tf.nn.dropout(embedded_sen + pos, keep_prob), scale_emb_sen, bias_emb_sen)

            with tf.variable_scope('encoder'):
                for block in range(self.config.block):
                    with tf.variable_scope('layer_' + str(block)):
                        with tf.variable_scope('attention'):
                            with tf.variable_scope('self'):
                                WQ = tf.layers.Dense(self.config.embedding_size, name='query')
                                WK = tf.layers.Dense(self.config.embedding_size, name='key')
                                WV = tf.layers.Dense(self.config.embedding_size, name='value')

                                Q = tf.concat(tf.split(WQ(now1), self.config.head, axis=-1), axis=0)
                            ...
                            with tf.variable_scope('output'):
                                ...
                                Z = WO(tf.concat(
                                    tf.split(tf.matmul(tf.nn.dropout(softmax(QK, mask_final), keep_prob), V),
                                             self.config.head,
                                             axis=0), axis=-1))
                                scale_sa = tf.get_variable('LayerNorm/gamma',
                                                           initializer=tf.ones([self.config.embedding_size],
                                                                               dtype=tf.float32))
                                bias_sa = tf.get_variable('LayerNorm/beta',
                                                          initializer=tf.zeros([self.config.embedding_size],
                                                                               dtype=tf.float32))
                                now1 = layer_norm_compute(now1 + tf.nn.dropout(Z, keep_prob), scale_sa, bias_sa)
                        ...
                        with tf.variable_scope('output'):
                            ...
                            scale_ff_sen = tf.get_variable('LayerNorm/gamma',
                                                           [self.config.embedding_size],
                                                           initializer=tf.ones_initializer(),
                                                           dtype=tf.float32)
                            bias_ff_sen = tf.get_variable('LayerNorm/beta',
                                                          [self.config.embedding_size],
                                                          initializer=tf.zeros_initializer(),
                                                          dtype=tf.float32)
                            now1 = layer_norm_compute(tf.nn.dropout(ff(now), keep_prob) + now1, scale_ff_sen,
                                                      bias_ff_sen)
        ...
        with tf.variable_scope('decoder'):
            ...
            mask_final = tf.logical_and(mask, padding_mask)
            with tf.variable_scope('embeddings'):
                scale_emb_pos = tf.get_variable('LayerNorm/gamma',
                                                [self.config.embedding_size],
                                                initializer=tf.ones_initializer(),
                                                dtype=tf.float32)
                bias_emb_pos = tf.get_variable('LayerNorm/beta',
                                               [self.config.embedding_size],
                                               initializer=tf.zeros_initializer(),
                                               dtype=tf.float32)
                now2 = layer_norm_compute(tf.nn.dropout(pos, keep_prob), scale_emb_pos, bias_emb_pos)

            for block in range(self.config.block):
                with tf.variable_scope('layer_' + str(block)):
                    with tf.variable_scope('attention'):
                        with tf.variable_scope('mutual'):
                        ...
                        with tf.variable_scope('output'):
                            ...
                            scale_sa_pos = tf.get_variable('LayerNorm/gamma',
                                                           [self.config.embedding_size],
                                                           initializer=tf.ones_initializer(),
                                                           dtype=tf.float32)
                            bias_sa_pos = tf.get_variable('LayerNorm/beta',
                                                          [self.config.embedding_size],
                                                          initializer=tf.zeros_initializer(),
                                                          dtype=tf.float32)
                            now2 = layer_norm_compute(now2 + tf.nn.dropout(Z, keep_prob), scale_sa_pos, bias_sa_pos)
                    with tf.variable_scope('intermediate'):
                        ffrelu = tf.layers.Dense(4 * self.config.embedding_size,
                                                 activation=gelu,
                                                 kernel_initializer=tf.truncated_normal_initializer(stddev=0.02),
                    ...
                    with tf.variable_scope('output'):
                        ...
                        scale_ff_pos = tf.get_variable('LayerNorm/gamma',
                                                       [self.config.embedding_size],
                                                       initializer=tf.ones_initializer(),
                                                       dtype=tf.float32)
                        bias_ff_pos = tf.get_variable('LayerNorm/beta',
                                                      [self.config.embedding_size],
                                                      initializer=tf.zeros_initializer(),
                                                      dtype=tf.float32)

                        now2 = layer_norm_compute(tf.nn.dropout(ff(now), keep_prob) + now2, scale_ff_pos, bias_ff_pos)

        with tf.variable_scope('cls/predictions'):
            with tf.variable_scope('transform'):
                scale_pr = tf.get_variable('LayerNorm/gamma',
                                           [self.config.embedding_size],
                                           initializer=tf.ones_initializer(),
                                           dtype=tf.float32)
                bias_pr = tf.get_variable('LayerNorm/beta',
                                          [self.config.embedding_size],
                                          initializer=tf.zeros_initializer(),
                                          dtype=tf.float32)
                now1, now2 = tf.split(now2, 2)
                now = now1 + now2
        ...
        accuracy = tf.cast(tf.equal(prediction, sen[:,1:]), tf.float32)
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=sen[:,1:], logits=logits)
        ...
        train_op, tvars = create_optimizer(loss, self.config.lr, self.config.epochs, 10000)

        topk = tf.argsort(logits, axis=-1, direction='DESCENDING')[:, :, :pri]
        err1 = tf.reduce_sum(tf.cast(tf.equal(topk, tf.tile(tf.expand_dims(sen[:,1:], axis=2), [1, 1, pri])), tf.int32),
                             axis=-1)

        log_probs = tf.nn.log_softmax(logits, axis=-1)
        one_hot_labels = tf.one_hot(sen[:,1:], depth=self.char_dict_len, dtype=tf.float32)
