...
tf.flags.DEFINE_string('model_save_path', 'model/simplelm12_fix/', 'The path where model shall be saved')
tf.flags.DEFINE_string('checkpoint', 'simplelm.ckpt', 'The path where model shall be saved')
...
tf.flags.DEFINE_integer('warmup', 500000, 'number of warmup')
...
class SimpleLM():
    ...
    def build_model(self):
        ...
        # 随机(15%)将某些字修改为其他字
        noise = tf.cond(tf.less(keep_prob, 1.0),
                        lambda: tf.where(tf.less(tf.random.uniform(tf.shape(sen), 0.0, 1.0), 0.15),
                                         tf.cast(tf.random.uniform(tf.shape(sen), 1.0, self.char_dict_len), tf.int32),
                                         sen),
                        lambda: sen)
        ...
        with tf.variable_scope('bert'):
            ...
            mask = tf.logical_and(tf.tile(tf.logical_or(future_mask, past_mask), [self.config.head, 1, 1]),
                                  padding_mask)

            # 随机(5%)屏蔽某些字
            mask_final = tf.cond(tf.less(keep_prob, 1.0),
                                 lambda: tf.logical_and(mask,
                                                        tf.greater(tf.random.uniform(tf.shape(mask), 0.0, 1.0), 0.05)),
                                 lambda: mask)
            ...
            with tf.variable_scope('embeddings'):
                embedding_matrx = tf.get_variable('word_embeddings',
                                                  ...
                                                  dtype=tf.float32)
                pos_full = tf.get_variable('position_embeddings',
                                           shape=[self.config.maxword, self.config.embedding_size],
                                           initializer=tf.truncated_normal_initializer(stddev=0.02),
                                           dtype=tf.float32)
                pos = tf.tile(tf.expand_dims(pos_full[:max_ls], axis=0), [batch_size, 1, 1])

                embedded_sen = tf.nn.embedding_lookup(embedding_matrx, noise)
                ...
                now = layer_norm_compute(tf.nn.dropout(pos, keep_prob), scale_emb_sen, bias_emb_sen)
                    ...
                    with tf.variable_scope('layer_' + str(block)):
                            ...
                            with tf.variable_scope('output'):
                                ...
                                bias_sa = tf.get_variable('LayerNorm/beta',
                                                          initializer=tf.zeros([self.config.embedding_size],
                                                                               dtype=tf.float32))
                                now = layer_norm_compute(now + tf.nn.dropout(Z, keep_prob), scale_sa, bias_sa)
                        ...
                        with tf.variable_scope('output'):
                            ...
                            bias_ff_sen = tf.get_variable('LayerNorm/beta',
                                                          ...
                                                          initializer=tf.zeros_initializer(),
                                                          dtype=tf.float32)
                            now = layer_norm_compute(tf.nn.dropout(ff(now2), keep_prob) + now, scale_ff_sen,
                                                     bias_ff_sen)
        ...
        logits = tf.reshape(logits, [-1, max_ls, self.char_dict_len])[:, 1:-1]
        ...
        train_op = create_optimizer(loss, self.config.lr, self.config.epochs, self.config.warmup)
    ...
    def makepb(self):
        ...
        with tf.gfile.FastGFile("model/simplelmmodel12.pb", mode='wb') as f:
            f.write(const_graph.SerializeToString())


def main(unused_argv):
    char_dict = load_vocab('chinese_L-12_H-768_A-12/vocab.txt')

    simplelm = SimpleLM(CONFIG, char_dict)
    if CONFIG.mode == 'train0':
        if not os.path.exists(CONFIG.model_save_path):
            os.makedirs(CONFIG.model_save_path)
        simplelm.build_model()
        simplelm.train([
