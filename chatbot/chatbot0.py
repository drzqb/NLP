...
class Seq2Seq():
    ...
    def build_model(self):
        ...
        saver.save(sess, FLAGS.model_save_path + 'chatbot0')
        sess.close()
    ...
    def train(self, qtext2id, qtext2id_length, atext2id_input, atext2id_target, atext2id_length, qa_dict,
        ...
        new_saver = tf.train.import_meta_graph(FLAGS.model_save_path + 'chatbot0.meta')
        new_saver.restore(sess, FLAGS.model_save_path + 'chatbot0')
        ...
        for epoch in range(1, FLAGS.epochs + 1):
            ...
            if epoch % FLAGS.per_save == 0:
                new_saver.save(sess, FLAGS.model_save_path + 'chatbot0')
                print('Model saved successfully!')

        fig = plt.figure(figsize=(10, 8))
        plt.plot(loss_)
        plt.savefig(FLAGS.model_save_path + 'chatbot0_loss.png', bbox_inches='tight')
        plt.close(fig)

    def predict(self, qa_dict, qa_reverse_dict):
        sess = tf.Session()
        tf.train.import_meta_graph(FLAGS.model_save_path + 'chatbot0.meta').restore(sess,
                                                                                    FLAGS.model_save_path + 'chatbot0')
        graph = tf.get_default_graph()
